...
```{r compile-datasets}
# Merge non-CO2 data first (works)
no_co2_assign <- cn_all_clean %>%
  rbind(qpcr_prop_conc_norm, n_data_clean) %>%
...
# Merge CO2 only data (works)
co2_only_assign <- co2_data_clean %>%
  # Convert sampled_date to double
...
all_data_assign <- co2_only_assign %>%
  rbind(no_co2_assign) %>%
  select(-licor)
```
