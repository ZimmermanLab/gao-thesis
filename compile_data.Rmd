...
id_assignments <- read_csv("output/2022/jar_assignments/master_list.csv") %>%
  rename(sampling_week = drying_treatment) %>%
  # This is my attempt to recode it into something more sensical...TBD
  mutate(rewet = case_when(
    str_detect(pre_post_wet, "pre|all_dry|initial|cw") ~ "no",
    pre_post_wet == "post" ~ "yes")) %>%
  mutate(control = case_when(
    pre_post_wet == "cw" ~ "yes",
    .default = "no"))

# Match assignments with sampling dates
sampling_dates <- read_csv("output/2022/jar_assignments/sampling_dates.csv") %>%
  
id_assignments <- id_assignments %>%
  mutate(sampled_date = case_when(
