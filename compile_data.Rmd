...
all_data <- merge_datasets(ea_data = cn_all_clean,
                           ...
                           output_file = "output/2022/master_dataset_03.csv")
```

```{r flag-outliers}
# Isolate dataset to samples only
samples_only <- isolate_samples(all_data)

# Add outlier flags for any outlying tech reps
...
flagged_outliers <- flag_outliers(samples_only)
```

```{r collapse-tech-reps}
# Find mean across technical replicates, retaining all data
...
collapsed_samples <- collapse_tech_reps(flagged_outliers)

# Why is sample_id 012 (7 rows) missing all leachate data?
```
