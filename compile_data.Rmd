...
source("code/functions/co2_functions/pull_sample_dates.R")
...
qpcr_prop_conc_norm <- norm_soil_wt(clean_qpcr, dried_wts) %>%
  ...
  # and clean up for master dataframe merging, changing the value and units from
  # Cq to normalized proportional concentrations
  norm_conc()
...
# should really be replaced by `auc_calib_date` but the create_std_curve()
# function is broken right now. In this older file, 'auc_norm_date', the column
# "auc_norm" should really be labelled to "auc_calib" because they have been
# calibrated to the gas standards, not normalized.
...
```{r load-sample-ids}
# Read in treatment group sampling dates
sampling_dates <- read_csv("output/2022/jar_assignments/sampling_dates.csv")

id_assignments <- read_csv("output/2022/jar_assignments/master_list.csv") %>%
  # Code CO2 only jars
  ...
  # Add leading zeros to sample_id
...
```{r compile-datasets}
# Join all datasets
all_data <- cn_all_clean %>%
  rbind(qpcr_prop_conc_norm, n_data_clean, co2_data_clean) %>%
  mutate(sampled_date = as.double(sampled_date))

# Add id_assignments df to add treatment condition columns + sampled date
all_data <- all_data %>%
  left_join(id_assignments, c("sample_id", "sample_type"))
