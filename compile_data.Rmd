...
qpcr_normalized_wts <- norm_soil_wt(clean_qpcr, dried_wts)

# Calculate proportional concentrations
qpcr_prop_conc_wts <- calc_prop_conc(qpcr_normalized_wts)
...
qpcr_prop_conc_norm <- norm_conc(qpcr_prop_conc_wts)
...
```{r co2-clean}
# Note that this uses the old file, `auc_norm_date` for testing purposes, which should really be replaced by `auc_calib_date` but the create_std_curve() function is broken right now. In this older file, 'auc_norm_date', the column "auc_norm" should really be labelled to "auc_calib" because they have been calibrated to the gas standards, not normalized.
