...
cw_samples <- readr::read_csv("output/jar_assignments/w_cc_cw.csv") %>%
  ...
  rbind(
    ...
    mutate(drying_treatment = "cw", pre_post_wet = NA, cc_treatment = "no_cc")) %>%
  arrange(sample_no)
...
means_w_cc_no2_no3 <- data.frame(
  ...
  test <- gsub("mean_wk4_w_cc_|_no2_no3", "", means_w_cc_no2_no3)

means_w_cc_no2_no3 <- data.frame(lapply(means_w_cc_no2_no3, function(x) {
  c(gsub(("mean_wk4_w_cc_" && "_no2_no3"), "", x))
}))

ggplot(means_w_cc_no2_no3, aes(x = names, y = value)) +
  geom_col()
