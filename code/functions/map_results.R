...
add_results <- function(all_treatments, samples_only) {
  ...
  for (row in 1:length(samples_only$sample_no)) {
    sample_cc <- all_treatments$cc_treatment[which(
      grepl((samples_only$sample_no[row]), all_treatments$sample_no))]
    ...
    if (sample_cc == "no_cc" && sample_pre_post_wet == "pre" &&
      ...
      wk4_no_cc_pre <- rbind(wk4_no_cc_pre, samples_only[row, ])
    ...
    if (sample_cc == "w_cc" && sample_pre_post_wet == "pre" &&
        sample_drying_treatment == "four_wk") {
      wk4_w_cc_pre <- rbind(wk4_w_cc_pre, samples_only[row, ])
    ...
    if (sample_cc == "no_cc" && sample_pre_post_wet == "post"
        && sample_drying_treatment == "four_wk") {
      wk4_no_cc_post <- rbind(wk4_no_cc_post, samples_only[row, ])
    ...
    if (sample_cc == "w_cc" && sample_pre_post_wet == "post"
        && sample_drying_treatment == "four_wk") {
      wk4_w_cc_post <- rbind(wk4_w_cc_post, samples_only[row, ])
    ...
    if (sample_cc == "no_cc" && sample_pre_post_wet == "cw"
        && sample_drying_treatment == "four_wk") {
      wk4_no_cc_cw <- rbind(wk4_no_cc_cw, samples_only[row, ])
    ...
    if (sample_cc == "w_cc" && sample_pre_post_wet == "cw"
        && sample_drying_treatment == "four_wk") {
      wk4_w_cc_cw <- rbind(wk4_w_cc_cw, samples_only[row, ])
